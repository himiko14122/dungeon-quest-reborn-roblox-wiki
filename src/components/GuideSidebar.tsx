        ...
        <div className="rounded-2xl bg-white/[0.03] border border-white/[0.08] p-5 backdrop-blur-sm light-card-border card-accent-bar">
          ...
          <div className="space-y-2">
            {moreGuides.map((guide) => (
              <Link key={guide.href} href={guide.href} title={guide.title} className="group flex items-center gap-2 rounded-xl bg-white/[0.03] border border-white/[0.06] px-3 py-2.5 hover:border-[var(--color-accent)]/30 transition-all duration-200 light-card-border-subtle">
          ...
          <Link href={`/${contentType}`} title={t('guide_sidebar_all')} className="mt-4 text-xs font-semibold text-[var(--color-accent)] hover:underline flex items-center gap-1">
