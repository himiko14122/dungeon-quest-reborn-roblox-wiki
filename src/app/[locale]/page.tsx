...
import { getBaseMetadata, websiteJsonLd, organizationJsonLd, itemListJsonLd } from '@/lib/seo';
...
export default async function HomePage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
            {moduleCards.map((card) => {
              ...
              return (
                <Link key={card.key} href={card.href} title={card.title} className="group block rounded-2xl bg-white/[0.03] border border-white/[0.08] p-6 hover:border-[var(--color-accent)]/40 hover:shadow-[0_0_30px_rgba(233,69,96,0.1)] transition-all duration-300 backdrop-blur-sm card-accent-bar">
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          <div>
            <p className="text-[var(--color-text-secondary)] mb-6 leading-relaxed">
              Whether you are a newcomer taking your first steps into the Shadow Crypt or a veteran raider seeking the ultimate legendary weapon, our data-driven tier lists, comprehensive guides, and community-crafted strategies cover every aspect of the Dungeon Quest Reborn experience.
            </p>
            <div className="flex flex-wrap gap-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            {infoItems.map((item) => (
              <div key={item} className="px-4 py-3 rounded-xl bg-white/[0.03] border border-white/[0.08] backdrop-blur-sm">
                <p className="text-xs text-[var(--color-text-muted)]">{t(`home_overview_${item}`)}</p>
                <p className="font-bold text-sm text-[var(--color-text-primary)]">{t(`home_overview_${item}_val`)}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="bg-[var(--color-bg-secondary)] border-y border-[var(--color-border)] full-width-bg">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          ...
          <div className="rounded-2xl bg-white/[0.02] border border-white/[0.08] overflow-hidden backdrop-blur-sm">
            <div className="grid grid-cols-[2rem_1fr_5rem_1fr] sm:grid-cols-[2.5rem_1.5fr_5rem_1.5fr_1.5fr] gap-x-4 text-xs font-semibold text-[var(--color-text-muted)] uppercase tracking-wider border-b border-[var(--color-border)] px-4 py-3">
              <span></span><span>{t('home_weapon_name')}</span><span>{t('tierList_tierLabel')}</span><span className="hidden sm:inline">{t('home_weapon_type')}</span><span>{t('home_weapon_effect')}</span>
            </div>
            {WEAPONS.slice(0, 6).map((weapon, idx) => {
              const tc = tierColorVal(weapon.tier);
              return (
                <Link key={weapon.id} href="/weapons" title={t(weapon.nameKey)} className="grid grid-cols-[2rem_1fr_5rem_1fr] sm:grid-cols-[2.5rem_1.5fr_5rem_1.5fr_1.5fr] items-center gap-x-4 px-4 py-3 border-b border-white/[0.04] last:border-b-0 hover:bg-[var(--color-accent)]/5 transition-colors group">
                  ...
                  <span className="flex items-center gap-2 font-semibold text-sm group-hover:text-[var(--color-accent)] transition-colors">
                    <Swords className="w-4 h-4 text-[var(--color-accent)] flex-shrink-0" />
                    {t(weapon.nameKey)}
                  </span>
                  ...
                  <span className="hidden sm:inline text-xs text-[var(--color-text-secondary)]">{t(weapon.typeKey)}</span>
                  <span className="text-xs text-[var(--color-text-secondary)]">{t(weapon.sourceKey)}</span>
                </Link>
              );
            })}
          </div>
      ...
      {/* ===== SPELL CARDS (Data-Driven Cards 1) ===== */}
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="flex items-center justify-between mb-8">
          <div>
            <div className="section-label">{t('home_module_spells')}</div>
            <h2 className="text-2xl md:text-3xl font-bold font-[var(--font-heading)] gradient-text">{t('home_spell_cards_title')}</h2>
          </div>
          <Link href="/spells" title={t('home_module_spells')} className="text-sm font-semibold text-[var(--color-accent)] hover:underline flex items-center gap-1">{t('view_all')} <ArrowRight className="w-3 h-3" /></Link>
        </div>
        <p className="text-[var(--color-text-secondary)] mb-6 leading-relaxed">{t('home_spell_cards_desc')}</p>
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {SPELLS.map((spell) => {
            const tc = spell.tier === 'S' ? 'var(--color-tier-s)' : spell.tier === 'A' ? 'var(--color-tier-a)' : 'var(--color-tier-b)';
            return (
              <Link key={spell.id} href="/spells" title={t(spell.nameKey)} className="category-card group block rounded-2xl overflow-hidden">
                <div className="p-5">
                  <div className="flex items-center gap-2 mb-3">
                    <Sparkles className="w-4 h-4 text-[var(--color-accent)]" />
                    <span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ color: tc, background: `${tc}15` }}>{spell.tier}</span>
                  </div>
                  <h3 className="text-[0.9375rem] font-bold font-[var(--font-heading)] text-[var(--color-text-primary)] group-hover:text-[var(--color-accent)] transition-colors mb-1">{t(spell.nameKey)}</h3>
                  <p className="card-meta text-[0.75rem] text-[var(--color-text-secondary)] mb-2"><span className="meta-label">{t('home_spell_class')}: </span><span className="meta-value">{t(spell.classKey)}</span></p>
                  <p className="card-desc text-[0.8125rem] text-[var(--color-text-secondary)] mb-2">{t(spell.effectKey)}</p>
                  <p className="text-[0.75rem] text-[var(--color-accent)] font-semibold">{t('home_spell_best')}: {t(spell.bestKey)}</p>
                </div>
              </Link>
            );
          })}
      ...
      {/* ===== DUNGEON TABLE (Data-Driven Table 2) ===== */}
      <section className="bg-[var(--color-bg-secondary)] border-y border-[var(--color-border)] full-width-bg">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="flex items-center justify-between mb-8">
            <div>
              <div className="section-label">{t('home_module_dungeons')}</div>
              <h2 className="text-2xl md:text-3xl font-bold font-[var(--font-heading)] gradient-text">{t('home_dungeon_table_title')}</h2>
            </div>
            <Link href="/dungeons" title={t('home_module_dungeons')} className="text-sm font-semibold text-[var(--color-accent)] hover:underline flex items-center gap-1">{t('view_all')} <ArrowRight className="w-3 h-3" /></Link>
          </div>
          <p className="text-[var(--color-text-secondary)] mb-6 leading-relaxed">{t('home_dungeon_table_desc')}</p>
          <div className="rounded-2xl bg-white/[0.02] border border-white/[0.08] overflow-hidden backdrop-blur-sm">
            <div className="grid grid-cols-[2rem_1fr_5rem_1fr_1fr] sm:grid-cols-[2.5rem_1.5fr_5rem_1.5fr_1.5fr] gap-x-4 text-xs font-semibold text-[var(--color-text-muted)] uppercase tracking-wider border-b border-[var(--color-border)] px-4 py-3">
              <span></span><span>{t('home_dungeon_name')}</span><span>{t('tierList_tierLabel')}</span><span>{t('home_dungeon_level')}</span><span>{t('home_dungeon_boss')}</span>
            </div>
            {DUNGEONS.map((dungeon, idx) => {
              const tc = tierColorVal(dungeon.tier);
              return (
                <Link key={dungeon.id} href="/dungeons" title={t(dungeon.nameKey)} className="grid grid-cols-[2rem_1fr_5rem_1fr_1fr] sm:grid-cols-[2.5rem_1.5fr_5rem_1.5fr_1.5fr] items-center gap-x-4 px-4 py-3 border-b border-white/[0.04] last:border-b-0 hover:bg-[var(--color-accent)]/5 transition-colors group">
                  <span className="text-xs font-mono text-[var(--color-text-muted)]">{idx + 1}</span>
                  <span className="flex items-center gap-2 font-semibold text-sm group-hover:text-[var(--color-accent)] transition-colors">
                    <Gamepad2 className="w-4 h-4 text-[var(--color-accent)] flex-shrink-0" />
                    {t(dungeon.nameKey)}
                  </span>
                  <span className="justify-self-start text-[10px] font-bold px-1.5 py-0.5 rounded-full" style={{ color: tc, background: `${tc}15` }}>{dungeon.tier}</span>
                  <span className="text-xs text-[var(--color-text-secondary)]">{t(dungeon.levelKey)}</span>
                  <span className="text-xs text-[var(--color-text-secondary)]">{t(dungeon.bossKey)}</span>
                </Link>
              );
            })}
          </div>
      ...
      {/* ===== CLASS CARDS (Data-Driven Cards 2) ===== */}
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="flex items-center justify-between mb-8">
          <div>
            <div className="section-label">{t('home_module_classes')}</div>
            <h2 className="text-2xl md:text-3xl font-bold font-[var(--font-heading)] gradient-text">{t('home_class_cards_title')}</h2>
          </div>
          <Link href="/classes" title={t('home_module_classes')} className="text-sm font-semibold text-[var(--color-accent)] hover:underline flex items-center gap-1">{t('view_all')} <ArrowRight className="w-3 h-3" /></Link>
        </div>
        <p className="text-[var(--color-text-secondary)] mb-6 leading-relaxed">{t('home_class_cards_desc')}</p>
        <div className="grid sm:grid-cols-2 gap-5">
          {CLASSES.map((cls) => {
            const tc = cls.tier === 'S' ? 'var(--color-tier-s)' : cls.tier === 'A' ? 'var(--color-tier-a)' : 'var(--color-tier-b)';
            return (
              <Link key={cls.id} href="/classes" title={t(cls.nameKey)} className="category-card group block rounded-2xl overflow-hidden">
                <div className="p-5">
                  <div className="flex items-center gap-2 mb-3">
                    <TrendingUp className="w-4 h-4 text-[var(--color-accent)]" />
                    <span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ color: tc, background: `${tc}15` }}>{cls.tier}</span>
                  </div>
                  <h3 className="text-lg font-bold font-[var(--font-heading)] text-[var(--color-text-primary)] group-hover:text-[var(--color-accent)] transition-colors mb-1">{t(cls.nameKey)}</h3>
                  <p className="card-meta text-[0.75rem] text-[var(--color-text-secondary)] mb-2"><span className="meta-label">{t('home_class_role')}: </span><span className="meta-value">{t(cls.roleKey)}</span></p>
                  <p className="card-meta text-[0.75rem] text-[var(--color-text-secondary)] mb-2"><span className="meta-label">{t('home_class_weapon')}: </span><span className="meta-value">{t(cls.weaponKey)}</span></p>
                  <p className="card-desc text-[0.8125rem] text-[var(--color-text-secondary)]"><span className="meta-label">{t('home_class_spell')}: </span><span className="meta-value">{t(cls.spellKey)}</span></p>
                </div>
              </Link>
            );
          })}
        ...
        <div className="max-w-3xl mx-auto space-y-3">
          {faqKeys.map((key) => (
            <details key={key} className="rounded-2xl bg-white/[0.03] border border-white/[0.08] p-5 backdrop-blur-sm group">
              ...
              <p className="mt-3 text-sm text-[var(--color-text-secondary)] leading-relaxed">{t(`faq_${key}_a`)}</p>
            </details>
          ))}
        </div>
      </section>
