...
export async function generateStaticParams() {
  // NOTE: only generate content/article routes here. Non-content nav items
  // (about/sitemap/privacy-policy/terms-of-service) have dedicated static
  // pages (`src/app/[locale]/<key>/page.tsx`) that render them; generating
  // their paths here collides with those pages and causes non-deterministic
  // out/ output (one build serves the real page, another serves notFound()).
...
export default async function CatchAllPage({ params }: { params: Promise<{ locale: string; slug: string[] }> }) {
  ...
  if (isContentType(first)) {
    ...
    const GUIDE_STOP_WORDS = new Set(['dungeon', 'quest', 'reborn', 'roblox', 'game', 'boss', 'warrior', 'mage', 'weapon', 'spell', 'armor', 'guide', 'tier', 'list', 'update', 'class', 'equipment', 'wiki', 'complete', 'ultimate', 'best', 'build', 'tips', 'walkthrough', 'the', 'a', 'an', 'and', 'or', 'in', 'on', 'at', 'to', 'for', 'is', 'it', 'how', 'what', 'all', 'vs']);
